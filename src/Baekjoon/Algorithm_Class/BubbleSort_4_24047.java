...
public class BubbleSort_4_24047 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(quotient == 0) {  // 몫이 0인 경우 그냥 깡으로 bubbleSort 전범위 적용
            indicator = bubbleSort(arr, 0, remainder, n, k, bw);
        } else{
            for(turn = 0; turn < quotient; turn++){ // 몫이 0이 아닌 경우 범위별로 bubblesort 실행
                // 몫만큼 반복, 마지막 시행은 remainder만큼만 실행
                if(indicator) break; // 시행이 끝나기 이전에 swapNo == k 만족시 true를 반환하기 때문에 반복문 정지
                if(remainder != 0 && turn == quotient - 1) {  // 마지막 시행 (나머지만큼)
                        indicator = bubbleSort(arr, turn * 5, (turn + 1) * 5 + remainder, n, k, bw);
                    break; // 정지 (나머지 0인 경우 조건문 패스하고 바로 밑의 줄 실행하게 됨 바로 아래줄 한번 더 실행)
                }
                    indicator = bubbleSort(arr, turn * 5, (turn + 1) * 5, n,  k, bw);
            }
        }
    ...
    static boolean bubbleSort(int[] target, int start, int end, int n , int k, BufferedWriter bw) throws IOException {
